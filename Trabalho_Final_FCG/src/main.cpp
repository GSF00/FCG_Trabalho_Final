...
float g_ViewPhi = -3.141592/4;//0.0f;
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "INF01047 - 00288913/XXXXXXXX - Guilherme Schaab Fisch/XXXXXXXXXXXXXXXXX", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        else
        {
            ...
            camera_position_c = glm::vec4(g_TorsoPositionX - g_CameraDistance/2*direcao.x, 3, g_TorsoPositionZ - g_CameraDistance/2*direcao.z, 1.0f);
...
//        DrawVirtualObject("the_face");
        ...
        model = Matrix_Translate(2.0f,1.0f,0.0f)*Matrix_Scale(2.0f, 2.0f, 2.0f);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        ...
        PushMatrix(model);
            ...
            model = model * Matrix_Scale(0.8f, 1.0f, 0.3f);
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(-0.55f, 0.2f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço direito
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.5f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço direito
                    ...
                    PushMatrix(model);
                        ...
                        model = model * Matrix_Translate(0.0f,-0.8f,0.0f)*Matrix_Scale(1.0f, 1.0f, 1.0f)*Matrix_Rotate_Y(3.141592/2)*Matrix_Rotate_Z(3.141592/2);
                        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
                        glUniform1i(g_object_id_uniform, GUN);
                        DrawVirtualObject("Cube.007_Cube.012");
       ...
       PushMatrix(model); // Guardamos matriz model atual na pilha
        ...
        PushMatrix(model);
        /*
            model = model * Matrix_Translate(0.0f, 0.8f, 0.0f);
            model = model * Matrix_Scale(0.3f, -0.3f, 0.3f);
            model = model * Matrix_Rotate_X(0.8f*3.141592); // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
        */
        ...
        // PERNA DIREITA
        PushMatrix(model);
            model = model * Matrix_Translate(-0.225f, -1.05f, 0.0f);
            PushMatrix(model);
                model = model * Matrix_Scale(0.35f, 0.75f, 0.3f);
                glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                DrawVirtualObject("the_cube");
            PopMatrix(model);
            ...
            PushMatrix(model); // Canela
                model = model * Matrix_Translate(0.0f, -0.8f, 0.0f);
                PushMatrix(model);
                    model = model * Matrix_Scale(0.3f, 0.75f, 0.2f);
                    glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawVirtualObject("the_cube");
                PopMatrix(model);
            //PopMatrix(model);
                ...
                PushMatrix(model); // Pé
                    model = model * Matrix_Translate(0.0f, -0.5f, 0.11f);
