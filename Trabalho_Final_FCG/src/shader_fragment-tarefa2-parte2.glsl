...
void main()
{
    ...
    vec4 l = normalize(vec4(0.0f, 2.0f, 1.0f, 0.0f));

    // Abertura da fonte "Spotlight", a = 30º
    float a = 3.141592 / 6;

    // Direção da fonte "Spotlight"
    vec4 v_spotlight = normalize(vec4(0.0f, -1.0f, 0.0f, 0.0f));

    // Vetor que vai da fonte "Spotlight" até o ponto atual
    vec4 p_spotlight = normalize(vec4(position_world.x - 0.0f,position_world.y - 2.0f,position_world.z - 1.0f, 0.0f));
    ...
    if(dot(v_spotlight, p_spotlight) >= cos(a))
